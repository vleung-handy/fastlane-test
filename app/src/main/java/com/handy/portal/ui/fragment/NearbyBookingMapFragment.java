...
public class NearbyBookingMapFragment extends SupportMapFragment implements OnMapReadyCallback
{
    ...
    HashMap<Marker, NearbyJobMapMarker> markersHashMap = new HashMap<>();
    ArrayList<NearbyJobMapMarker> markersArray = new ArrayList<>();


    public static NearbyBookingMapFragment newInstance(final Booking booking, Booking.BookingStatus status, ZipClusterPolygons polygons)
    {
        NearbyBookingMapFragment fragment = new NearbyBookingMapFragment();
    ...
    public void onMapReady(GoogleMap map)
    {
        ...
        //TODO: Do a custom thing when custom marker is clicked
    ...
    private void addCustomMarkers(GoogleMap map)
    {
        ...
        List<LatLng> testMarkers = new ArrayList<>();

        testMarkers.add(new LatLng((center.latitude + 0.001f), (center.longitude + 0.001f)));
        testMarkers.add(new LatLng((center.latitude + 0.002f), (center.longitude + 0.002f)));
        testMarkers.add(new LatLng((center.latitude + 0.003f), (center.longitude + 0.003f)));

        //add the test markers that are clickable

        // Initialize the HashMap for Markers and MyMarker object
        markersArray.add(new NearbyJobMapMarker("Abra", "icon1", testMarkers.get(0)));
        markersArray.add(new NearbyJobMapMarker("Bone", "icon2", testMarkers.get(1)));
        markersArray.add(new NearbyJobMapMarker("Czech", "icon3", testMarkers.get(2)));

        for (NearbyJobMapMarker myMarker : markersArray)
        {

            // Create user marker with custom icon and other options
            MarkerOptions markerOption = new MarkerOptions().position(myMarker.getLatLng());
            markerOption.icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_alarm_clock));

            Marker currentMarker = map.addMarker(markerOption);
            markersHashMap.put(currentMarker, myMarker);

            //As you can see, we also call mMap.setInfoWindowAdapter(new MarkerInfoWindowAdapter());.
            // //Of course your application will crush if you run it, since we don't have this MarkerInfoWindowAdapter class declared. We are gonna use this as the Adapter to our custom marker InfoWindow. This way we are gonna create a custom layout for the InfoWindow.
            map.setInfoWindowAdapter(new NearbyJobMarkerInfoWindowAdapter());
        }
    ...
    public class NearbyJobMarkerInfoWindowAdapter implements GoogleMap.InfoWindowAdapter
    {
        public NearbyJobMarkerInfoWindowAdapter()
        {
        ...
        {
            View v = getActivity().getLayoutInflater().inflate(R.layout.nearby_job_map_marker, null);

            ImageView markerIcon = (ImageView) v.findViewById(R.id.nearby_job_map_marker_icon);
            TextView markerLabel = (TextView) v.findViewById(R.id.nearby_job_map_marker_label);

            NearbyJobMapMarker myMarker = markersHashMap.get(marker);
            if (myMarker != null)
            {
            ...
            {
                markerIcon.setImageResource(R.drawable.ic_alarm_clock);
                markerLabel.setText("no data");
            }
