    ...
    protected void initFromBooking(Booking booking, Bundle arguments)
    {
        ...
        if (fullDetails)
        {
            if (booking.getBookingInstructions() != null && booking.getBookingInstructions().size() > 0)
            {
                ...
                sectionView.init(applicationContext.getString(R.string.customer_details), R.drawable.circle_green, entries, true);
            }
        }
        ...
        if (booking.getExtrasInfo() != null && booking.getExtrasInfo().size() > 0)
        {
            ...
            //TODO: Hardcoding string and icon, we need to get this data from the booking info
            sectionView.init(applicationContext.getString(R.string.extras), R.drawable.circle_green, entries, true);
        }
        ...
        if (fullDetails)
        {
            ...
            //TODO: Hardcoding string and icon, we need to get this data from the booking info
            sectionView.init(applicationContext.getString(R.string.customer_request), R.drawable.circle_green, entries, false);
        }
    }

    private BookingDetailsJobInstructionsSectionView addSection(LinearLayout instructionsLayout)
    {
        LayoutInflater.from(applicationContext).inflate(R.layout.element_booking_details_job_instructions_section, instructionsLayout);
